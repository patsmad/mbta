...
class MapTest(unittest.TestCase):
    def getRoute(self, letter):
        return {
            'attributes': {'long_name': 'Route {}'.format(letter)},
            'id': 'Route {} ID'.format(letter),
            'relationships': {'line': {'data': {'id': 'Line {} ID'.format(letter)}}}
        }

    def getStop(self, letter):
        return {'attributes': {'name': 'Stop {}'.format(letter)}, 'id': 'Stop {} ID'.format(letter)}

    def getLine(self, letter):
        return {'attributes': {'long_name': 'Line {}'.format(letter)}, 'id': 'Line {} ID'.format(letter)}

    def setUp(self):
        routes = {letter: self.getRoute(letter) for letter in ['A', 'B', 'C']}
        stops = {letter: self.getStop(letter) for letter in ['A', 'B', 'C', 'D', 'E', 'F', 'G']}
        mock_stop_response = {
            'Route A ID': [stops[letter] for letter in ['A', 'B', 'C', 'D']],
            'Route B ID': [stops[letter] for letter in ['E', 'B', 'F']],
            'Route C ID': [stops[letter] for letter in ['G']]
        }
        mock_line_response = {
            'Line A ID': self.getLine('A'),
            'Line B ID': self.getLine('B'),
            'Line C ID': self.getLine('C')
        }
        self.mock_map = build_map(MockMBTAAPI(list(routes.values()), mock_stop_response, mock_line_response))
        self.empty_map = build_map(MockMBTAAPI([], [], []))
    ...
    def test_connecting_routes(self):
        ...
        self.assertIn('Route A', self.mock_map.stops_to_routes['Stop B'])
        self.assertIn('Route B', self.mock_map.stops_to_routes['Stop B'])
    ...
    def test_same_station_path(self):
        ...
        self.assertEqual(same_station_path[0], 'Route A')
    ...
    def test_same_route_path(self):
        ...
        self.assertEqual(same_route_path[0], 'Route A')
    ...
    def test_different_route_path(self):
        ...
        self.assertIn('Route A', different_route_path)
        self.assertIn('Route B', different_route_path)
    ...
    def test_crossing_route_path(self):
        ...
        self.assertEqual(crossing_route_path[0], 'Route B')
